...
public class GetServerTimeHelper {
	private String mDateAndTime = null;
	private boolean isGet = false;

	public GetServerTimeHelper(final String host) {
		new Thread() {
			public void run() {
				SntpClient client = new SntpClient();
				if (client.requestTime(host, 30000)) {
	...
	public class SntpClient {
		...
		private long read32(byte[] buffer, int offset) {
			...
			return ((long) i0 << 24) + ((long) i1 << 16) + ((long) i2 << 8)
					+ (long) i3;
			/*
			 * return ((long) i0 << 24) + ((long) i1 << 16) + ((long) i2 << <img
			 * class="wp-smiley" alt="8)" src=
			 * "http://www.imobilebbs.com/wordpress/wp-includes/images/smilies/icon_cool.gif"
			 * > + (long) i3;
			 */
		}
